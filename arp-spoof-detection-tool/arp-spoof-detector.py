...
def alert_box(message):
    print("Under Attack", "You are under arp attack by \n MAC ADDRESS: " + message)
        ...
        try:
            ...
            if real_mac != response_mac:  # Compares the real_mac with packet_mac
                ...
                print("\r[-] You are under attack by " + packet[
                    scapy.ARP].hwsrc + ", Total Spoof packet received = " + str(total_packet))
                total_packet = total_packet + 1  # Updates the packet count

                #Write on logs file
                #f1.write("\r[-] You are under attack by " + packet[
                    #scapy.ARP].hwsrc + ", Total Spoof packet received = " + str(total_packet))
                datos = {
                    'Mac attacker' : packet[scapy.ARP].hwsrc,
                    'Packets recieved' : total_packet
